...
public class RandomSlotAssignment implements SlotAssignmentStrategy {
    private ParkingLotRepository parkingLotRepository;
    private ParkingSlotRepository parkingSlotRepository;

    public  RandomSlotAssignment(ParkingLotRepository parkingLotRepository,ParkingSlotRepository parkingSlotRepository){
    ...
    public Slot assignSlot(VehicleType vehicleType, Gate gate) {
        ...
        for (Slot slot : allParkingSlots) {
            if(slot.getStatus().equals(SlotStatus.AVAILABLE) && slot.getSupportedVehicleTypes().contains(vehicleType)){
                return slot;
            }
        }
            return null;
